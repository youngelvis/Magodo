...
import 'package:magodo/pages/admin_page/zonal_superadmin_vehicle/zonalUpdateVehicle.dart';
import 'package:magodo/pages/admin_page/zonal_superadmin_vehicle/zonal_superAmin_vehicleCard.dart';
import 'package:magodo/services/services.dart';
import 'package:pull_to_refresh/pull_to_refresh.dart';
import '../../../api/api.dart';
import '../../../components/roundedDropDownTextfield.dart';
import '../../../components/title.dart';
import '../../resident_Page/form_pages_for_residents/register_vechicle/update_registration_vehicle.dart';
...
class ZonalAdminVehicle extends StatefulWidget {
  ResidentModel? data;

  ZonalAdminVehicle({Key? key, this.data}) : super(key: key);

  @override
  State<ZonalAdminVehicle> createState() =>
      _ZonalAdminVehicleState();
}
...
class _ZonalAdminVehicleState
    extends State<ZonalAdminVehicle> {
  var totalRfid = 0;
  var zone;
  String? status;
  final statusOptions = [
  ...
  List<FetchZonalSuperAdminVehicle> vehicles = [];
  ...
  Future<bool> getVehicle({bool isRefresh = false}) async {
    ...
    var data = await Services().adminVehicleReport(
        startDate: '',
        startEnd: '',
        page: currentPage,
        search: '',
    ...
    final result = fetchZonalSuperAdminVehiclesFromJson(data);
  ...
  Future _filterFunction() async => debounce(() async {
    int page = 0;
    var data = await Services().adminVehicleReport(
        startDate: _startDate.text.isEmpty ? '' : _startDate.text,
        startEnd: _startEnd.text.isEmpty ? '' : _startEnd.text,
        page: page,
        search: '',
        status: status ?? '',
        zone: widget.data?.zone ?? "");
                                  ...
                                  DeleteUpdateButton(
                                    ...
                                    onPressedUpdateButton: () {
                                      Navigator.push(
                                          ...
                                          MaterialPageRoute(
                                              builder: (context) =>
                                              ZonalSuperAdminUpdateVehicleRegistration(
                                                    vehicleData: vehicle,
                                                    data: widget.data,
                                                  )));
                                    },
